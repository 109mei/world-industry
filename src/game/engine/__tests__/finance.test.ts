...
/**
 * 採石作業員を n 人まとめて雇えるだけの元手。
 * 1人目が15万円で、雇うたびに12%ずつ高くなるので、人数ぶんを合計してから渡す。
 * まとめ買いは全額そろわないと1人も雇えないので、端数で失敗しないよう人件費ぶんも足しておく。
 */
function wageTestCash(workers: number): number {
  return facilityBulkCost(FACILITY_MAP.worker_stone, 0, workers) + 1_000_000;
}

describe('人件費', () => {
  it('従業員を雇うと毎秒お金がかかる', () => {
    const e = makeEngine(wageTestCash(4));
    expect(wagePerSec(e.state)).toBe(0);
    expect(e.buyFacility('worker_stone', 4)).toBe(4);
  ...
  it('止めている施設の人にも給料は出る', () => {
    const e = makeEngine(wageTestCash(2));
    expect(e.buyFacility('worker_stone', 2)).toBe(2);
...
describe('赤字と倒産', () => {
  it('所持金がマイナスになると利息が付き、猶予のあいだは続けられる', () => {
    const e = makeEngine(wageTestCash(10));
    expect(e.buyFacility('worker_stone', 10)).toBe(10);
  ...
  it('赤字が続くと倒産し、最初からになる', () => {
    const e = makeEngine(wageTestCash(3));
