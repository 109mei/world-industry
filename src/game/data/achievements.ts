...
export const ACHIEVEMENTS: AchievementDef[] = [
  ...
  { id: 'place_collector', name: '街を買い集める', description: '地図から買った場所が10か所になる。', icon: 'icon_terrain_city', check: (s) => Object.keys(s.estate?.custom ?? {}).length >= 10 },
  ...
  { id: 'sap_tapper', name: '樹液採り', description: '樹液を100個集める。', icon: 'icon_chemical_resin', check: (s) => (s.stats.totalObtained['sap'] ?? 0) >= 100 },
  { id: 'woodworker', name: '木工職人', description: '家具を10個作る。', icon: 'icon_office_chair', check: (s) => (s.stats.totalProduced['furniture'] ?? 0) >= 10 },
  { id: 'chemist', name: '化学者', description: '化学薬品を1,000個作る。', icon: 'icon_material_chemical', check: (s) => (s.stats.totalProduced['chemical'] ?? 0) >= 1000 },
  ...
  { id: 'top100', name: '番付の常連', description: '長者番付で30位以内に入る。', icon: 'icon_ui_crown', check: (_s, d) => d.assets >= 1_400_000_000_000 },
  ...
  { id: 'automated', name: 'ぜんぶ自動で', description: '自動化を6種類そろえる。', icon: 'icon_part_robot_arm', check: (s) => ['auto_gather', 'auto_craft', 'auto_deliver', 'auto_pitch', 'auto_survey', 'auto_build'].filter((k) => (s.prestige?.upgrades?.[k] ?? 0) > 0).length >= 6 },
